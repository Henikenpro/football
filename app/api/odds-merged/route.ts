  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (res.status === 429) {
        ...
        console.warn(
          `[fetchWithRetryRaw] 429 for ${url}, waiting ${waitMs}ms then retry.`
        );
        await sleep(waitMs);
        throw new Error("429");
      }
...
async function fetchAllOddsByDate_untilCovered(
  date: string,
  neededFixtureIds: Set<number>,
  maxPages = 100
) {
  const all: any[] = [];
  let page = 1;
  ...
  while (true) {
    if (page > Math.min(maxPages, MAX_PAGE_LIMIT)) {
      console.warn(
        `[fetchAllOddsByDate_untilCovered] reached page limit ${page - 1}`
      );
      break;
    }
    const url = `${API_HOST}/odds?date=${encodeURIComponent(
      date
    )}&page=${page}`;
    let jsonWrap: any;
    try {
      ...
      console.warn(
        `[fetchAllOddsByDate_untilCovered] fetch page ${page} failed:`,
        e.message ?? e
      );
    }
    ...
    if (covered.size >= neededFixtureIds.size) {
      console.log(
        `[fetchAllOddsByDate_untilCovered] covered all needed fixtures at page ${page}`
      );
      break;
    }
    if (
      covered.size >= Math.min(neededFixtureIds.size, MAX_FIXTURES_TO_DISPLAY)
    ) {
      console.log(
        `[fetchAllOddsByDate_untilCovered] covered target display count ${covered.size} at page ${page}`
      );
      break;
    }
    ...
    if (paging) {
      if (
        ...
        totalPages = Math.ceil(
          paging.total / (paging.limit || respArr.length || 1)
        );
...
async function fetchOddsPerFixtures(
  fixtureIds: number[],
  limit = FALLBACK_PER_FIXTURE_LIMIT
) {
  const results: any[] = [];
    ...
    async (fid) => {
      try {
        ...
        if (Array.isArray(json?.response) && json.response.length) {
         ...
         const e = err as Error;
        console.warn(
          `[fetchOddsPerFixtures] fixture ${fid} fetch failed:`,
           e.message ?? e
        );
      }
    },
...
function buildOddsMapFromResponses(oddsResponses: any[]) {
  ...
  for (const item of oddsResponses) {
    ...
    if (Array.isArray(item.bookmakers)) bookmakers = item.bookmakers;
      ...
      bookmakers = [
        { ...(item.bookmaker || {}), bets: item.bets ?? item.markets ?? [] },
      ];
    } else if (item.provider && Array.isArray(item.provider.markets)) {
      bookmakers = [
        {
          name: item.provider.name ?? item.provider.id,
          bets: item.provider.markets,
        },
      ];
    } else if (
      item.name &&
      (item.bets || item.markets || item.values || item.options)
    ) {
      bookmakers = [item];
    }
    ...
    const normalized = bookmakers
      .map((bm: any) => {
        const bets = Array.isArray(bm.bets)
          ? bm.bets
          : Array.isArray(bm.markets)
          ? bm.markets
          : Array.isArray(bm.options)
          ? bm.options
          : [];
        return {
...
function pickFixturesToProcess(
  allFixtures: any[],
  maxCount = MAX_FIXTURES_TO_DISPLAY
) {
  ...
  try {
    if (!API_KEY) {
      return NextResponse.json(
        { ok: false, error: "FOOTBALL_API_KEY not set" },
        { status: 500 }
      );
    }
    ...
    if (!date) {
      return NextResponse.json(
        { ok: false, error: "Missing required 'date' query parameter" },
        { status: 400 }
      );
    }

    const cacheKey = `odds-merged:${date}:${timezone ?? "default"}:${
      leagueParam ?? "all"
    }`;
    ...
    try {
      ...
      return NextResponse.json(
        { ok: false, error: "Failed to fetch fixtures" },
        { status: 500 }
      );
    }
    ...
    if (!Array.isArray(fixtures) || fixtures.length === 0) {
      const empty = {
        ...
        merged: [],
        count: 0,
        debug: { note: "no fixtures for date" },
      };
    ...
    const selectedFixtures = pickFixturesToProcess(
      fixtures,
      MAX_FIXTURES_TO_DISPLAY
    );
    const selectedFixtureIds = selectedFixtures
      .map((f: any) => f.fixture?.id ?? f.id)
    ...
    try {
      const {
        ...
        coveredSet,
        pagesFetched: pf,
      } = await fetchAllOddsByDate_untilCovered(date, neededFixtureSet, 100);
      oddsResponses = all;
      pagesFetched = pf;
      // If we've covered all or a large portion, great. Otherwise fallback below.
    } catch (err) {
      console.warn(
        "[odds-merged] bulk odds fetch had errors, will run fallback",
        err
      );
      oddsResponses = [];
    }
    ...
    if (missingIds.length > 0) {
      ...
      console.log(
        `[odds-merged] ${missingIds.length} selected fixtures missing odds -> fallback per-fixture for up to ${limit}`
      );
      const fallbackResponses = await fetchOddsPerFixtures(
        missingIds.slice(0, limit),
        limit
      );
      if (fallbackResponses.length > 0)
        oddsResponses.push(...fallbackResponses);
    }
      ...
      sample: (m.odds || []).slice(0, 2).map((b: any) => ({
        ...
        betsSample: (b.bets || [])
          ...
          .map((t: any) => ({
            ...
            values: (t.values || [])
              .slice(0, 4)
              .map((v: any) => v.odd ?? v.price ?? v.value),
          })),
      })),
    }));
