...
export default function Page() {
  ...
  const isFinishedStatus = (s?: string) => {
    ...
    const up = s.toUpperCase();
    return (
      up === "FT" || up === "AET" || up === "PEN" || up === "AW" || up === "AP"
    );
  };
  ...
  const isUpcomingStatus = (s?: string) => {
    ...
    return (
      ...
      up === "TBA" ||
      up === "SUSP" ||
      (!isLiveStatus(up) && !isFinishedStatus(up))
    );
  };
  ...
  const grouped = filteredMatches.reduce((acc, m) => {
    ...
    if (!acc[key])
      acc[key] = {
        meta: { leagueName, country, flag: m.league?.flag },
        items: [] as APIFixture[],
      };
  ...
  const groupsArray = Object.entries(grouped).map(([key, val]) => ({
    key,
    ...val,
  }));
    ...
    <div className="p-4 max-w-4xl mx-auto">
      ...
      {!loading && !error && filteredMatches.length === 0 && (
        <div className="text-center text-gray-500 py-8">
          Không có trận đấu nào cho ngày này.
        </div>
      )}
              ...
              <div>
                {itemsSorted.map((m, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-14 font-medium text-gray-800">
                        {time}
                      </div>
